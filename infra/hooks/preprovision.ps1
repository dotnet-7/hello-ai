...
if (![string]::IsNullOrEmpty($env:GITHUB_WORKSPACE)) {  
    ...
    $DIR = Split-Path -Parent -Path (Resolve-Path -LiteralPath "$0")  
    & "$DIR/login.sh"  
}  
...
if ($WORKSPACE -eq "default" -and ($null -eq $env:FORCE_TERRAFORM_REMOTE_STATE_CREATION -or $env:FORCE_TERRAFORM_REMOTE_STATE_CREATION -eq "true")) {  
...
if (!(Test-Path -Path $TF_WORKSPACE_DIR)) {  
    New-Item -ItemType Directory -Path $TF_WORKSPACE_DIR  
}  
